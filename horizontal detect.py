        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    box_length= xyxy[2] - xyxy[0]
                    scaled_width = im0.shape[1]
                    width_ratio = (box_length / scaled_width) * 100
                    distance_from_last=(((xyxy[1]+xyxy[3])/2)-Y_of_centre[-1])
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        c = int(cls)  # integer class
                        write_to_csv(p.name, label, confidence_str, width_ratio)
                        label = None if hide_labels else (names[c] if hide_conf else f"{names[c]} {conf:.2f} length ={box_length:.2f},W ={distance_from_last/640}")
                        annotator.box_label(xyxy, label, color=colors(c, True))
                    if save_crop:
                        save_one_box(xyxy, imc, file=save_dir / "crops" / names[c] / f"{p.stem}.jpg", BGR=True)
                    Y_of_centre.append((xyxy[1]+xyxy[3])/2)
            # Stream results
